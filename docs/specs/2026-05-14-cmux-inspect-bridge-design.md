...
2. **点发送**：拼好 `InspectItem` push 进 `window.__cmuxInspectQueue` + sessionStorage 同步 + 浮层关闭
...
- **启动**：`setsid + nohup cmux-inspect-bridge watch-daemon &`（双 fork detached）
- **pid 文件**：`~/.cmux-inspect/watcher.pid`，含 `pid|target|started_at`
- **log**：`~/.cmux-inspect/watcher.log`，stdout/stderr 重定向到此
- **状态查询**：`--status` 读 pid 文件 + `kill -0 pid` 验活
- **停止**：`--stop` 读 pid → `kill TERM` → 等 5s → `kill KILL` 兜底
...
- `init`：完整初始化（默认）
- `inject [--surface ...]`：仅注入到指定 surface（用于新开 browser 后补注入）
- `uninject [--surface ...]`：清理注入
- `doctor`：诊断当前 workspace 状态（browser surface 数 / 注入状态 / daemon 状态）
